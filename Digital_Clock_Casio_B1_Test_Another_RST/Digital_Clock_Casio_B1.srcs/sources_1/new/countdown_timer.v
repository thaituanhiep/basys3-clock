module countdown (
  ...
  reg [5:0] mem_sec, mem_min;   // ⭐ bộ nhớ mềm
  reg set_sel;                  // 0 = SEC, 1 = MIN

  // detect entering mode 4
  reg cd_en_d;
  wire enter_cd_mode = cd_enable && !cd_en_d;
  ...
  always @(posedge clk) begin
    ...
    if (!cd_enable) begin
      // KHÔNG reset giá trị - chỉ dừng FSM
      running  <= 1'b0;
      finished <= 1'b0;
      state    <= CD_SET;
      set_sel  <= 1'b0;
      ...
      // ⭐ khi vào mode 4 → nạp giá trị đã lưu
      if (enter_cd_mode) begin
        sec <= mem_sec;
        min <= mem_min;
      end
  ...
  // =============================
  // INIT - chỉ chạy 1 lần khi bật board
  // =============================
  initial begin
    sec      = 6'd5;
    min      = 6'd0;
